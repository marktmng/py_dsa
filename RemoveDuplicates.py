...
class LinkedList:
    ...
    def remove_duplicates(self):
        values = set() # create a set to store unique values
        current = self.head # start from the head of the list
        prev = None # to keep track of the previous node
        while current is not None: #traverse the entire list
            if current.value in values: # check if the value is already seen
                prev.next = current.next # duplicate found: skip the current node
                self.length -= 1 # decrement the length of the list
            else:
                values.add(current.value) # first time seeing this value, add to the 
